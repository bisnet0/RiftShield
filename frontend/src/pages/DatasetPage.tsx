...
export default function DatasetPage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      showToast({
        ...
        message: t("ds.imagem_adicionada"),
        type: "success",
      });
      setFile(null);
      setPreview(null);
      ...
      showToast({
        title: t("geral.erro"),
        message: err?.response?.data?.error || t("ds.falha_upload"),
        type: "error",
      });
    } finally {
      setUploading(false);
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      showToast({
        title: t("geral.atencao"),
        message: t("ds.entrada_excluida"),
        type: "info",
      });
      loadData();
    } catch {
    ...
    <Box w="full" maxW="1800px" mx="auto" pb={10}>
      <VStack spacing={8} align="stretch">
        <Flex justify="space-between" align="center" wrap="wrap" gap={4}>
          <Heading size="lg" color={appFx.textColor}>
            {t("ds.title")}
          </Heading>
          <Button
            size="sm"
        ...
        >
          <Heading size="md" mb={4} color={appFx.textColor}>
            {t("ds.upload")}
          </Heading>
          <VStack spacing={4}>
            <input
            ...
            >
              ...
              <Text color={appFx.textColor}>
                {file ? file.name : t("ds.clique_selecionar")}
              </Text>
            </Box>
        ...
        >
          <Flex justify="space-between" align="center" mb={4}>
            <Heading size="md" color={appFx.textColor}>
              {t("ds.entradas")} ({entries.length})
            </Heading>
            <HStack>
              <Select
                size="sm"
          ...
          {entries.length === 0 && (
            <Text color={appFx.textMuted} textAlign="center" py={8}>
              {t("ds.sem_entradas")}
            </Text>
          )}
                ...
                <Flex justify="space-between" align="start">
                  <VStack align="start" spacing={1}>
                    ...
                    <HStack>
                      ...
                      <Badge colorScheme={e.augmented ? "cyan" : "gray"}>
                        {e.source}
                      </Badge>
                    </HStack>
                    <Text fontSize="xs" color={appFx.textMuted}>
                      {e.labels?.length || 0} label(s)
                    </Text>
                  </VStack>
                  <Button
                    size="xs"
